...
const stores = [
  ...
  {
    name: "custom list",
    key: "harbor.customlists.v1.profile-a",
    baseKey: "harbor.customlists.v1",
    otherKey: "harbor.customlists.v1.profile-b",
...
test("context list creation persists the initial item in one write and preserves existing metadata", () => {
  const key = "harbor.customlists.v1.profile-a";
...
test("factory list operations use the official profile key and keep manga stores independent", () => {
  const movieStore = lists.createListStore("harbor.customlists.v1");
  const mangaStore = lists.createListStore("harbor.mangalists.v1");
  const movieKey = "harbor.customlists.v1.profile-a";
  const mangaKey = "harbor.mangalists.v1.profile-a";
  const initial = [{ id: "same-id", name: "Kept", description: "keep description", items: [] }];
  storage.values.set(movieKey, JSON.stringify(initial));
  storage.values.set(mangaKey, JSON.stringify(initial));
  const manga = { id: "source::manga", type: "manga", name: "Manga" };
  let updates = 0;
  const stop = mangaStore.subscribeLists(() => updates++);
  assert.equal(mangaStore.setListMembership("same-id", manga, true, profile).status, "added");
  assert.equal(updates, 1);
  assert.equal(mangaStore.readLists()[0].items[0].type, "manga");
  assert.equal(mangaStore.readLists()[0].description, "keep description");
  assert.deepEqual(movieStore.readLists()[0].items, []);
  assert.deepEqual(storage.writes, [mangaKey]);
  assert.equal(storage.values.has("harbor.mangalists.profile-a"), false);
  assert.equal(storage.values.has("harbor.customlists.profile-a"), false);
  stop();
});

test("factory creation and rename persist descriptions atomically and reject failed or stale writes", () => {
  const store = lists.createListStore("harbor.mangalists.v1");
  const key = "harbor.mangalists.v1.profile-a";
  const created = store.createListWithItem(
    "Reading",
    { ...item, type: "manga" },
    profile,
    "  Notes  ",
  ...
  assert.deepEqual(storage.writes, [key]);
  assert.equal(
    store.renameListWithResult(created.id, "Renamed", profile, "New notes").status,
    "updated",
  );
  const saved = storage.values.get(key);
  assert.equal(store.readLists()[0].description, "New notes");
  storage.failure = new Error("Blocked");
  assert.deepEqual(store.renameListWithResult(created.id, "Lost", profile, "Lost notes"), {
    status: "error",
    reason: "storage-failed",
  });
  ...
  storage.failure = null;
  storage.values.set(profilesKey, JSON.stringify({ ...profiles, activeId: "profile-b" }));
  assert.deepEqual(store.deleteListWithResult(created.id, profile), {
    status: "error",
    reason: "profile-changed",
  });
  assert.equal(storage.values.get(key), saved);
