...
function harness(initial: DownloadItem[] = []) {
  ...
  const native = { beforeDelete: undefined as ((path: string) => Promise<void>) | undefined };
  ...
  const invoke = async (command: string, args: { path: string; protectedPaths?: string[] }) => {
    ...
    await native.beforeDelete?.(args.path);
...
test("confirmed batch scope cannot follow a replacement path on a later download", async () => {
  const initial = [item("one"), item("two")];
  const h = harness(initial);
  const entered = deferred();
  const resume = deferred();
  h.native.beforeDelete = async (path) => {
    if (path === initial[0].path) {
      entered.resolve();
  ...
  const expectedScope = new Map(
    initial.map((entry) => [
      entry.id,
      { path: entry.path, kind: entry.kind, format: entry.format },
    ]),
  );
  const deleting = h.store.runDownloadBatch(["one", "two"], "delete", { expectedScope });
  await entered.promise;
  const replacement = "C:/fixtures/unapproved-replacement.mkv";
  h.store.downloadById("two")!.path = replacement;
  h.files.set(replacement, { isFile: true, canonicalPath: replacement.toLowerCase() });
  resume.resolve();
  const result = await deleting;
  assert.deepEqual(result.succeeded, ["one"]);
  assert.equal(result.failed[0]?.id, "two");
  assert.match(result.failed[0]?.error ?? "", /changed|scope/i);
  assert.deepEqual(h.removed, [initial[0].path]);
  assert.equal(h.files.has(replacement), true);
});

test("deletion rechecks the confirmed actor after waiting for a native writer", async () => {
  const h = harness();
  const path = "C:/fixtures/actor.mkv";
  const id = await h.store.enqueueDownload({
    meta: { id: "fixture", type: "movie", name: "Fixture" },
    url: "https://fixture.invalid/movie",
    destinationPath: path,
  });
  h.files.set(`${path}.part`, { isFile: true, canonicalPath: `${path}.part`.toLowerCase() });
  let current = true;
  const snapshot = h.store.downloadById(id)!;
  const deleting = h.store.runDownloadBatch([id], "delete", {
    isValid: () => current,
    expectedScope: new Map([[id, { path, kind: snapshot.kind, format: snapshot.format }]]),
  });
  ...
  assert.match(result.failed[0]?.error ?? "", /profile changed/i);
  assert.deepEqual(h.removed, []);
  assert.ok(h.store.downloadById(id));
});
