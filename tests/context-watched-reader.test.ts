...
test("mapped episode list projects one provider read into source-local coordinates", async () => {
  const h = harness();
  h.traktOnly();
  const state = await h.reader.readContextWatchedSnapshot({
    meta: { ...meta, id: "kitsu:123", type: "series" },
    imdbId: "tt123",
    episodeProjection: [
      { season: 1, episode: 1, imdbSeason: 4, imdbEpisode: 6 },
      { season: 1, episode: 2, imdbSeason: 4, imdbEpisode: 7 },
    ],
  });
  assert.deepEqual(state.providers[0]?.watched, new Set(["1:2"]));
  assert.deepEqual(h.calls, ["Trakt"], "mapping a list does not read the provider once per row");
});

test("a loaded source-local projection does not fetch or summarize a different full title", async () => {
  const h = harness();
  h.traktOnly();
  const state = await h.reader.readContextWatchedSnapshot({
    meta: { id: "kitsu:123", type: "series", name: "Fixture" },
    imdbId: "tt123",
    episodeProjection: [
      { season: 1, episode: 2, imdbSeason: 4, imdbEpisode: 7, airDate: "2020-01-01" },
      { season: 1, episode: 3, imdbSeason: 4, imdbEpisode: 8, airDate: "2999-01-01" },
    ],
  });
...
test("episode projection cannot invent missing coordinates or mix another source", async () => {
  for (const bad of [
    { season: 1, episode: 2 },
    { season: 1, episode: 2, sourceMetaId: "kitsu:456", imdbSeason: 4, imdbEpisode: 7 },
    { season: 1, episode: 2, imdbId: "tt999", imdbSeason: 4, imdbEpisode: 7 },
  ]) {
    const h = harness();
    h.traktOnly();
    const state = await h.reader.readContextWatchedSnapshot({
      meta: { ...meta, id: "kitsu:123", type: "series" },
      imdbId: "tt123",
      episodeProjection: [{ season: 1, episode: 1, imdbSeason: 4, imdbEpisode: 6 }, bad],
    });
...
test("native anime projection leaves Simkl source-local coordinates intact", async () => {
  const h = harness();
  h.simklOnly();
  const state = await h.reader.readContextWatchedSnapshot({
    meta: { ...meta, id: "mal:123", type: "series" },
    episodeProjection: [{ season: 1, episode: 2, imdbSeason: 4, imdbEpisode: 7 }],
  });
  assert.deepEqual(state.providers[0]?.watched, new Set(["1:2"]));
});
