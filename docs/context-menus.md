...
| Continuation | Global Continue last watched is a normal root row outside the player. It targets the last actual playback and saved tracks. A Continue Watching card independently targets its own entry. Local-card dismissal preserves its file and local resume position; remote/provider entries retain Harbor's existing history and progress-reset semantics. |
