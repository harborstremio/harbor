...
export function useRenderedContentContext() {
  ...
  return (event: MouseEvent<HTMLElement>) => {
    ...
    const isValid = () => container.isConnected && container.innerHTML === markup;
    ...
    if (mention && event.currentTarget.contains(mention)) {
      ...
      if (handle && /^[a-z\d_.-]+$/i.test(handle)) {
        const target = userContextTarget(handle);
        open(event, { ...target, isValid: () => isValid() && target.isValid?.() !== false });
      }
      return;
    }
