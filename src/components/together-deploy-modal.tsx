          ...
          <ol className="flex flex-col gap-3">
            ...
            <Step n={3}>
              ...
              <span className="mt-2 block text-[12px] text-ink-subtle">
                <Interpolate
                  text={t("Token name can be anything. The permission row must be exactly {b1} + {b2} + {b3}.")}
                  values={{
                    b1: <b>{t("Account")}</b>,
      ...
      {phase === "done" && result && (
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 rounded-xl border border-edge bg-canvas/60 p-4">
            ...
            >
              <span key={copied ? "copied" : "idle"} className="flex items-center gap-2 animate-copy-swap">
                {copied ? (
                  <>
    ...
    return (
      <div className="flex flex-col gap-4">
        ...
        <ol className="flex flex-col gap-2.5 text-[13px] text-ink">
          ...
          <Step n={4}>
            <Interpolate
              text={t("Come back here and hit {b1}. The Hello World can stay where it is. It's free and harmless.")}
              values={{
...
function linkifyMessage(text: string): React.ReactNode {
  ...
  return parts.map((part, i) =>
    /^https?:\/\//.test(part) ? (
      <button key={i} onClick={() => openUrl(part)} className="underline underline-offset-2 hover:text-ink">
        {part}
      </button>
    ) : (
