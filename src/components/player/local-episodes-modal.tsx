...
export function LocalEpisodesModal() {
  ...
  return (
    <GridModal key={state.payload.tmdbId ?? state.payload.imdbId ?? state.payload.title} payload={state.payload} />
  );
}
  ...
  const localEps = useMemo(
    () =>
      all
        .filter(
          (e) =>
            e.type === "show" && ((tmdbId != null && e.tmdbId === tmdbId) || (imdbId != null && e.imdbId === imdbId)),
        )
        ...
        <div className="flex flex-col gap-4 overflow-y-auto p-4">
          ...
          <div className="flex shrink-0 flex-col gap-1">
            {listEps.map((ep) => {
              ...
              const ratio = pr && ep.runtime && ep.runtime > 0 ? Math.min(1, pr.ms / (ep.runtime * 60_000)) : 0;
                ...
                >
                  ...
                  {ratio > 0.01 && (
                    <span className="absolute inset-x-0 bottom-0 h-[2px] bg-edge">
                      <span className="block h-full bg-accent" style={{ width: `${Math.max(2, ratio * 100)}%` }} />
                    </span>
                  )}
                </button>
              );
