...
export function ThemeBackdrop() {
  ...
  const dim = Math.max(0, Math.min(1, settings.theme.backgroundDim ?? preset?.background?.dim ?? 0.65));
  ...
  const isGradient =
    img.startsWith("linear-gradient") || img.startsWith("radial-gradient") || img.startsWith("conic-gradient");
  ...
  return (
    <>
      <div
        ...
        style={isGradient ? { background: img } : { backgroundImage: `url(${img})` }}
      />
      {!presetGradient && (
        <div className="pointer-events-none fixed inset-0 -z-20" style={{ background: "black", opacity: 0.45 }} />
      )}
      <div
