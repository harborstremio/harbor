...
export type ContextWatchedInput = {
  ...
  /** Read-only projection for a loaded source-local episode list, never a write scope. */
  episodeProjection?: readonly PlayEpisode[];
};
...
export async function readContextWatchedSnapshot(
  ...
  const mappedKey =
    selected && input.imdbId
      ...
      : localKey;
  const projection = input.episodeProjection;
  const verifiedProjection =
    !!projection?.length &&
    !!input.imdbId &&
    projection.every(
      (ep) =>
        (!ep.sourceMetaId || ep.sourceMetaId === id) &&
        (!ep.imdbId || ep.imdbId.split(":")[0] === input.imdbId) &&
        Number.isInteger(ep.season) &&
        ep.season >= 0 &&
        Number.isInteger(ep.episode) &&
        ep.episode > 0 &&
        Number.isInteger(ep.imdbSeason) &&
        ep.imdbSeason! >= 0 &&
        Number.isInteger(ep.imdbEpisode) &&
        ep.imdbEpisode! > 0,
    );
  const normalize = (keys: Set<string>) => {
    if (localKey && mappedKey) return new Set(keys.has(mappedKey) ? [localKey] : []);
    if (verifiedProjection)
      return new Set(
        projection!.flatMap((ep) =>
          keys.has(episodeKey(ep.imdbSeason!, ep.imdbEpisode!))
            ? [episodeKey(ep.season, ep.episode)]
            : [],
        ),
      );
    return keys;
  ...
  if (!movie && !selected && !input.episodeScope && !projection && !videos?.length) {
