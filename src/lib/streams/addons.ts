...
export async function fetchAddonStreams(
  ...
  for (let i = 0; i < addons.length; i++) {
    ...
    if (isPluginAddon(addon)) {
      const pluginIds = pickIds(addon, req.type, req.ids, req.animeIdUnverified === true, ranker);
      const pluginId = pluginIds[0] ?? pickIdByDeclaredTypes(addon, req.ids, ranker)?.id;
      if (!pluginId) {
