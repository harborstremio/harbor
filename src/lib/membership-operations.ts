...
export function performMembershipOperation(
  ...
  const itemId =
    operation.mode === "add" || operation.mode === "set"
      ? operation.item.id
      : operation.mode === "create"
        ? operation.container.items[0]?.id
        : operation.itemId;
  if (
    ((operation.mode === "add" || operation.mode === "set" || operation.mode === "move") &&
      !isId(operation.destinationId)) ||
    (operation.mode === "set" && !expected) ||
  ...
  const source =
    operation.mode === "move" || operation.mode === "remove"
      ...
      : undefined;
  if ((operation.mode === "move" || operation.mode === "remove") && !source) {
  ...
  const item = operation.mode === "add" || operation.mode === "set" ? operation.item : sourceItem;
  ...
  if (operation.mode === "set" && !operation.present) {
    if (!alreadyPresent) return { status: "unchanged" };
    destination.items = destination.items.filter((entry) => entry.id !== itemId);
    destination.updatedAt = Date.now();
    return persistMembershipSnapshot(key, containers, "removed");
  }
  if (alreadyPresent && (operation.mode === "add" || operation.mode === "set"))
  ...
  return persistMembershipSnapshot(key, containers, operation.mode === "move" ? "moved" : "added");
}
...
export function readMembershipSnapshot(
  ...
  // Collections retain their legacy key; list stores append the profile to the full key.
  const key =
    current.profile.activeId && !current.profile.settingsLinked
      ? `${baseKey === "harbor.collections.v1" ? "harbor.collections" : baseKey}.${current.profile.activeId}`
      : baseKey;
  let raw: string | null;
  try {
...
export function performContainerOperation(
  ...
  operation: { id: string; expectedRaw?: string | null } & (
    | { mode: "delete" }
    | { mode: "rename"; name: string; description?: string }
  ),
  ...
  if (operation.mode === "rename") {
    if (
      ...
      return { status: "error", reason: "invalid-data" };
    const description = operation.description?.trim() || undefined;
    if (
      container.name === operation.name &&
      (operation.description === undefined || container.description === description)
    )
      return { status: "unchanged" };
    container.name = operation.name;
    if (operation.description !== undefined) container.description = description;
