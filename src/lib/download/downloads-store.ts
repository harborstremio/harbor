...
export type DownloadFileScope = Pick<DownloadItem, "path" | "kind" | "format">;
export type DownloadDeleteOptions = {
  protectedPaths?: () => string[];
  expectedScope?: ReadonlyMap<string, DownloadFileScope>;
  isValid?: () => boolean;
};

function validateDownloadScope(id: string, options: DownloadDeleteOptions) {
  if (options.isValid?.() === false)
    throw new Error("The active profile changed. Open the menu again.");
  if (!options.expectedScope) return;
  const expected = options.expectedScope.get(id);
  const current = items.get(id);
  if (
    !expected ||
    !current ||
    ...
    throw new Error("This download's file scope has changed. Review the action again.");
}
...
export async function removeDownload(
  ...
  options = {
    ...
    expectedScope:
      ...
      new Map([[id, { path: item.path, kind: item.kind, format: item.format }]]),
  };
  validateDownloadScope(id, options);
  removing.add(id);
  rebuild();
  try {
    cancelDownload(id);
