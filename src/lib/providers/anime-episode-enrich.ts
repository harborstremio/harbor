...
import { meta as fetchCinemetaMeta, settleWithin } from "@/lib/cinemeta";
...
export const CINEMETA_EPISODE_ENRICH_DEADLINE_MS = 8_000;
...
async function enrichCinemetaEpisodes(
  ...
  if (
    episodes.every(
      (ep) => ep.thumbnail && ep.synopsis && !isGenericEpisodeTitle(ep.title, ep.number),
    )
  )
    return;
  const m = await settleWithin(
    fetchCinemetaMeta("series", imdbId),
    CINEMETA_EPISODE_ENRICH_DEADLINE_MS,
  ).catch(() => null);
...
export function startEpisodeEnrichment(
  episodes: KitsuEpisode[],
  settings: Settings,
  kitsuId: number,
  imdbId: string | null = null,
): { titlePromise: Promise<void>; completePromise: Promise<void> } {
  const titlePromise = enrichCinemetaEpisodes(episodes, imdbId);
  ...
  return { titlePromise, completePromise };
}
