...
export function sortGames(games: SportsGame[]): SportsGame[] {
  return games
    .slice()
    .sort(
      (a, b) => rank(a.state) - rank(b.state) || (a.state === "post" ? b.startMs - a.startMs : a.startMs - b.startMs),
    );
}
...
export async function fetchMatchSummary(leagueTag: string, eventId: string): Promise<SportsMatchDetail | null> {
  ...
  if (def.group === "combat") {
    ...
    const [homeProfile, awayProfile] = await Promise.all([fetchProfile(homeRaw.id), fetchProfile(awayRaw.id)]);
