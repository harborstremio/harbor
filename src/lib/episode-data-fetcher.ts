 ...
 */
async function extractTmdbId(seriesId: string, _seriesMeta: Meta, tmdbKey: string): Promise<number | null> {
  ...
  if (seriesId.startsWith("tt")) {
    try {
      ...
      const response = await get<{ tv_results?: Array<{ id: number }> }>(tmdbKey, `find/${seriesId}`, {
