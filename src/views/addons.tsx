...
export function AddonsView() {
  ...
  useEffect(() => {
    ...
    void import("@/lib/providers/stremio-addons-index").then((m) => m.ensureCommunityIndex().catch(() => undefined));
  }, []);
  ...
  const installed = useMemo(() => {
    ...
    return allAddons
      ...
      .sort(
        (a, b) =>
          (rank.get(a.transportUrl) ?? Number.MAX_SAFE_INTEGER) - (rank.get(b.transportUrl) ?? Number.MAX_SAFE_INTEGER),
      );
  }, [allAddons]);
...
function RemoteOrLocalDetail({
  ...
  useEffect(() => {
    ...
    setFailed(false);
    (async () => {
      const { communityFor, ensureCommunityIndex } = await import("@/lib/providers/stremio-addons-index");
