  ...
  return (
    <div className="flex h-full flex-col bg-canvas pb-8">
          ...
          <div className="flex w-full items-center justify-center gap-4 md:gap-12">
            <div className="flex flex-1 flex-col items-center gap-4 text-center">
              <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-elevated/40 p-3 shadow-xl ring-1 ring-edge-soft/50 backdrop-blur-sm md:h-32 md:w-32 md:p-5">
                {game.home.logo ? (
                  <img src={game.home.logo} className="h-full w-full object-contain drop-shadow-md" alt="" />
                ) : (
            ...
            <div className="flex flex-1 flex-col items-center gap-4 text-center">
              <div className="flex h-24 w-24 items-center justify-center rounded-2xl bg-elevated/40 p-3 shadow-xl ring-1 ring-edge-soft/50 backdrop-blur-sm md:h-32 md:w-32 md:p-5">
                {game.away.logo ? (
                  <img src={game.away.logo} className="h-full w-full object-contain drop-shadow-md" alt="" />
                ) : (
                  <div className="h-full w-full rounded-full bg-canvas" />
                )}
              </div>
      ...
      <div className="mx-auto mt-8 flex w-full max-w-4xl shrink-0 gap-6 border-b border-edge-soft/50 px-6">
        {(isCombat ? (["summary", "profile", "stats"] as const) : (["summary", "lineups", "stats"] as const)).map(
          (tId) => (
            <button
              key={tId}
              onClick={() => setTab(tId)}
            ...
            >
              {t(tId)}
              {tab === tId && <div className="absolute inset-x-0 bottom-0 h-0.5 rounded-t-full bg-brand" />}
            </button>
          ),
        )}
      </div>
      ...
      {detail.events.map((e, i) => {
        ...
        return (
          <div key={i} className="flex items-center gap-4 rounded-xl border border-edge-soft/30 bg-elevated/40 p-4">
            <div className="flex w-12 shrink-0 items-center justify-center font-bold text-ink-muted">{e.time}</div>
            {playerImage ? (
              <img
                src={playerImage}
        ...
        <div className="flex flex-col gap-8 md:flex-row">
          <div className="flex flex-1 flex-col gap-4">
            ...
            <TeamPitch
              ...
              teamAbbr={detail.home.abbr || "HOME"}
              isHome={true}
            />
          </div>
          <div className="flex flex-1 flex-col gap-4">
            ...
            <TeamPitch
              ...
              teamAbbr={detail.away.abbr || "AWAY"}
              isHome={false}
            />
          </div>
        </div>
          ...
          <div className="flex flex-col gap-2">
            {detail.homeRoster.map((p) => (
              <div key={p.id} className="flex items-center gap-3 text-sm">
                ...
                <span className={`flex-1 ${p.starter ? "font-bold text-ink" : "font-medium text-ink-muted"}`}>
                  {p.name}
                </span>
...
function StatRow({ label, hVal, aVal }: { label: string; hVal: string; aVal: string }) {
  ...
  return (
    <div className="flex items-center justify-between py-2 text-sm font-medium">
      ...
      <div className="flex-1 px-2 text-center text-xs tracking-wider text-ink-subtle uppercase">{t(label)}</div>
      <div
