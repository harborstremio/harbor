...
import { assertSocialActor, captureSocialActor } from "@/lib/social/action-actor";
import type {
  ActivityItem,
  Badge,
  Comment,
...
async function mutate(
  ...
  assertSocialActor(actor);
  if (!response.ok) throw new ProfileApiError(response.status);
  return response;
}
...
export async function postComment(
  ...
  const res = await mutate(
    `/u/${encodeURIComponent(handle)}/comments`,
    "POST",
...
export async function deleteComment(handle: string, id: string): Promise<void> {
  await mutate(`/u/${encodeURIComponent(handle)}/comments/${encodeURIComponent(id)}`, "DELETE");
}
...
export async function setCommentLike(
  ...
  const res = await mutate(
    `/u/${encodeURIComponent(handle)}/comments/${encodeURIComponent(id)}/like`,
    liked ? "POST" : "DELETE",
  );
