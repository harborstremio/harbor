...
export function SourceDiagnostic({
  ...
  const counts = useMemo(() => {
    ...
    for (const s of all) {
      const key = s.addonName ?? s.addonId ?? "";
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <span className="font-semibold text-ink-muted">{t("{n} cached", { n: cachedTotal })}</span>
        <span className="text-ink-subtle/40">·</span>
        <span>
          {counts.length === 1
            ? t("{total} found across 1 source", { total: totalRaw })
            : t("{total} found across {n} sources", { total: totalRaw, n: counts.length })}
        </span>
        <ChevronDown
          size={13}
          strokeWidth={2}
      ...
      {expanded && (
        <div className="flex flex-wrap items-center gap-x-2.5 gap-y-1 ps-1 text-[11px] text-ink-subtle/70">
          <span>{t("{n} kept after dedupe", { n: result.picker.all.length })}</span>
          <span className="text-ink-subtle/40">·</span>
          {counts.length === 0 ? (
            <span className="text-ink-subtle/60">{t("no sources returned anything")}</span>
          ) : (
            counts.map(([name, n], i) => (
              <span key={name || "__unknown"} className="flex items-center gap-1.5">
                {i > 0 && <span aria-hidden className="h-1 w-1 rounded-full bg-ink-subtle/40" />}
                <span>
                  {name || t("Unknown")} <span className="text-ink-subtle/45">{n}</span>
                </span>
              </span>
            ))
          )}
