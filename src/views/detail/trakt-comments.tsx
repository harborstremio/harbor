    ...
    <div className="flex gap-3 rounded-xl bg-elevated p-4 ring-1 ring-edge">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className="text-[13px] font-semibold text-ink">{comment.user.name ?? comment.user.username}</span>
        ...
        <div className="mt-2 flex items-center gap-3 text-[12px] text-ink-muted">
          ...
          {comment.replies > 0 && (
            ...
            >
              {loadingReplies ? <Loader2 size={12} className="animate-spin" /> : <MessageCircle size={12} />}
...
export function TraktComments({ resolution }: { resolution: IdResolution | null }) {
  ...
  useEffect(() => {
    if (!connected) return;
    let cancelled = false;
    traktRequest<{ username: string; images?: { avatar?: { full?: string } } }>("/users/me?extended=images")
      .then((data) => {
        if (cancelled) return;
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`flex items-center gap-1 rounded-lg px-3 py-1.5 text-[12px] font-medium ring-1 transition-colors ${
                myComments ? "bg-ink text-canvas ring-ink" : "text-ink-muted ring-edge hover:bg-elevated hover:text-ink"
              }`}
            >
              {t("My")}
            </button>
      ...
      <div className={`relative rounded-xl ${settings.blurComments && blurred ? "overflow-hidden" : ""}`}>
        ...
        {target && !connected && (
          <div className="mb-5 rounded-xl border border-edge-soft bg-elevated/60 p-5 text-center">
            <p className="text-[14px] text-ink-muted">{t("Connect your Trakt account to see comments and reviews.")}</p>
            <p className="mt-3">
              <button
        ...
        {target &&
          !loading &&
          (myComments ? comments.filter((c) => c.user.username === username) : comments).length === 0 &&
          myComments && <p className="text-[14px] text-ink-muted">{t("You haven't commented yet")}</p>}
