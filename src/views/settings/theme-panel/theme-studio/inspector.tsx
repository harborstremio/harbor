...
export function Inspector({
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
          ...
          {tab === "look" && (
            <div className="flex flex-col">
              <Group title="Identity" sub="What this theme is called.">
                <IdentityRow name={draft.name} blurb={draft.blurb} onChange={(p) => onPatch(p)} onSeed={onSeed} />
              </Group>
      ...
      {cardCssOpen && <CardCssPopout css={draft.css} onChange={onPatch} onClose={() => setCardCssOpen(false)} />}
    </div>
  );
}
...
function Group({
  ...
  return (
    <section className="border-b border-edge-soft last:border-b-0">
      ...
      >
        <span className="flex-1 text-[13px] font-bold uppercase tracking-[0.16em] text-ink-muted">{title}</span>
        <ChevronDown
          size={18}
          strokeWidth={2.4}
          className={`shrink-0 text-ink-subtle/70 transition-transform duration-200 ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
