  ...
  return (
    <>
      <div className="flex items-center justify-between">
        <p className="text-[16px] text-ink-subtle">
        ...
        {overrideCount > 0 && (
          <button
            onClick={resetAll}
            className="flex shrink-0 items-center gap-2 rounded-full border border-edge-soft bg-canvas/80 px-4 py-2 text-[15px] font-medium text-ink-muted transition-colors hover:border-edge hover:text-ink"
          >
            ...
            <div className="flex flex-col gap-6">
              {Array.from(subgroups.entries()).map(([groupName, items]) => (
                <div key={groupName} className="flex flex-col gap-1.5">
                  <h4 className="px-1 text-[13px] font-semibold uppercase tracking-[0.18em] text-ink-subtle">
                    {t(groupName)}
                  </h4>
                  {items.map((def) => {
...
function SeekStepRow({
  ...
  const t = useT();
  return (
    <div className="grid gap-5 rounded-xl border border-edge-soft bg-canvas/40 px-5 py-4 xl:grid-cols-[minmax(220px,1fr)_minmax(0,640px)] xl:items-center">
      <div className="flex min-w-0 flex-col gap-1">
        <span className="text-[17px] font-medium text-ink">{t("Seek step")}</span>
...
function SeekStepPicker({
  ...
  return (
    <div className="grid min-w-0 grid-cols-[90px_minmax(0,1fr)] items-center gap-3 rounded-lg border border-edge-soft bg-elevated/55 p-2">
      <span className="ps-2 text-[13px] font-semibold uppercase tracking-[0.14em] text-ink-subtle">
        {label}
      </span>
        ...
        {SEEK_STEP_OPTIONS.map((seconds) => {
          ...
          return (
            <button
              ...
              className={`h-9 min-w-0 rounded-[6px] px-1 font-mono text-[14px] font-bold tabular-nums transition-colors ${
                selected
...
function ReadOnlyHotkeyRow({
  ...
  const t = useT();
  return (
    <div className="flex items-center gap-5 rounded-xl border border-edge-soft bg-canvas/40 px-5 py-4">
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <span className="text-[17px] font-medium text-ink">{label}</span>
        <span className="text-[15px] text-ink-subtle">{description}</span>
      </div>
      <div className="flex h-10 min-w-[140px] items-center justify-center rounded-lg border border-edge bg-elevated px-4 text-[15px] font-semibold text-ink">
        {binding}
      </div>
...
function HotkeyRow({
  ...
  return (
    <div
      ref={rowRef}
      className={`flex items-center gap-5 rounded-xl border bg-canvas/40 px-5 py-4 transition-colors ${
        isCapturing ? "border-accent bg-accent/8" : "border-edge-soft"
      }`}
    >
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        <div className="flex items-center gap-2">
          <span className="text-[17px] font-medium text-ink">{t(def.label)}</span>
          {isCustom && !isCapturing && (
            <span className="rounded-full bg-accent/15 px-2 py-0.5 text-[11.5px] font-semibold uppercase tracking-[0.14em] text-accent">
              {t("Custom")}
            </span>
          )}
          {conflict && (
            <span className="rounded-full bg-danger/15 px-2 py-0.5 text-[11.5px] font-semibold uppercase tracking-[0.14em] text-danger">
              {t("Conflict")}
            </span>
          )}
        </div>
        <span className="text-[15px] text-ink-subtle">{t(def.description)}</span>
      </div>
      {isCapturing ? (
        <div className="flex items-center gap-2">
          <span className="rounded-md border border-accent/60 bg-accent/12 px-4 py-2 text-[15px] font-semibold uppercase tracking-[0.12em] text-accent">
            <Keyboard size={16} strokeWidth={2.4} className="me-1.5 inline-block" />
            {t("Press a key…")}
          </span>
          <button
        ...
        <div className="flex items-center gap-1.5">
          {isCustom && (
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-lg text-ink-subtle transition-colors hover:bg-raised hover:text-ink"
            >
          ...
          <button
            onClick={onStartCapture}
              className="flex h-10 min-w-[100px] items-center justify-center rounded-lg border border-edge bg-elevated px-4 text-[15px] font-semibold text-ink transition-colors hover:border-ink hover:bg-raised"
          >
