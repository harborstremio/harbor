    ...
    <>
          ...
          <div className="mt-4 flex flex-col gap-3">
            <HoverStyleGallery
              ...
              onChange={(style, customId) =>
                update(
                  customId != null ? { cardHoverStyle: style, customHoverId: customId } : { cardHoverStyle: style },
                )
              }
            />
      ...
      >
        <KeyField
          ...
          help={
            <>
              Highly recommended. This is what gives you the full Harbor experience: Popular, Trending, In Theaters, and
              per-service rails. Free at{" "}
              <ExtLink href="https://www.themoviedb.org/settings/api">themoviedb.org/settings/api</ExtLink>. Use the v3
              key, not the read access token.
            </>
          }
        />
        <ToggleRow
        ...
        <KeyField
          ...
          help={
            <>
              Free at <ExtLink href="https://www.omdbapi.com/apikey.aspx">omdbapi.com/apikey.aspx</ExtLink>. They email
              an activation link the first time. Click it, then come back and save.
            </>
          }
        />
        <KeyField
          ...
          help={
            <>
              Paid plan at <ExtLink href="https://ratingposterdb.com">ratingposterdb.com</ExtLink>. Once saved, every
              poster gets re-rendered with IMDb, Rotten Tomatoes, and Metacritic stamped on it.
            </>
          }
        />
        <KeyField
          ...
          help={
            <>
              Free key at <ExtLink href="https://mdblist.com/preferences/">mdblist.com</ExtLink>. Adds Letterboxd and
              Trakt community ratings to detail pages, covering what OMDb misses.
            </>
          }
        />
        <KeyField
        ...
        <KeyField
          ...
          help={
            <>
              ...
              <code>https://btttr.cc</code>), a bare RPDB-compatible server (your RPDB key is still sent), or a full URL
              template using <code>{"{imdbId}"}</code>, <code>{"{tmdbId}"}</code>, <code>{"{type}"}</code>, or{" "}
        ...
        <KeyField
          ...
          help={
            <>
              ...
              <ExtLink href="https://fanart.tv/get-an-api-key/">fanart.tv/get-an-api-key</ExtLink>. Use the "personal"
              key, not the project one.
            </>
          }
        />
        <KeyField
          ...
          help={
            <>
              Episode titles, alternate names, network info, and the arc/DVD/absolute orderings. Layered on TMDB so the
              better source wins per field. Free for personal use at{" "}
              <ExtLink href="https://thetvdb.com/api-information">thetvdb.com/api-information</ExtLink>
        ...
        <div className="mt-2 border-t border-edge-soft/60 pt-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <ToggleRow
              ...
              lockReason={!settings.omdbKey ? t("Add an OMDb key above to unlock this.") : undefined}
              note={settings.rpdbKey ? t("RPDB already paints scores onto the poster. Toggle to override.") : undefined}
            />
            <ToggleRow
          ...
          <p className="mt-2 text-[12px] text-ink-subtle">{t("{n} score badges enabled.", { n: enabledBadgeCount })}</p>
          <p className="mb-3 mt-6 text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-subtle">
            {t("Watchlist badge")}
          </p>
        ...
        <div className="flex items-center justify-between gap-4 rounded-xl bg-canvas/40 px-4 py-3.5">
          ...
          <div className="flex shrink-0 items-center gap-2">
            <input
              ...
              onChange={(e) => update({ localMinFileSizeMb: Math.max(0, Math.round(Number(e.target.value) || 0)) })}
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span className="max-w-[88%] text-[12px] leading-relaxed text-ink-muted">{opt.sub}</span>
            </div>
          </button>
        );
      })}
...
function RetentionPicker({ value, onChange }: { value: number; onChange: (v: number) => void }) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-ink-subtle">Keep frames for</p>
          ...
          return (
            <button
              ...
              className={`rounded-full px-3.5 py-1.5 text-[12px] font-semibold transition-colors ${
                selected ? "bg-ink text-canvas" : "bg-raised text-ink-muted hover:bg-elevated hover:text-ink"
              }`}
            >
              {opt.label}
            </button>
