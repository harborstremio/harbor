  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-[12.5px] text-ink-subtle">
        ...
        <span>
          {t("Active torrents")}{" "}
          <span className="font-mono text-accent">{status?.active_torrents ?? 0}</span>
        </span>
        {status?.dht_tier ? (
          <span>
            {t("DHT")}{" "}
            <span
              className={`font-mono ${(status.dht_nodes ?? 0) > 0 ? "text-accent" : "text-danger"}`}
            >
      ...
      <ToggleRow
        label={t("Keep stream downloads in the background")}
        sub={t(
        ...
        onChange={(value) => update({ keepStreamDownloadsInBackground: value })}
      />
