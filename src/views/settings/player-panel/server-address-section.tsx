...
import { remoteUiUrl } from "@/lib/remote/protocol";
...
export function ServerAddressSection() {
  ...
  useEffect(() => {
    ...
    void invoke<string | null>("lan_ip")
      ...
      .catch(() => {});
    void invoke<string>("remote_pair_token")
      .then((token) => {
        if (aliveRef.current) setPairToken(token);
      })
      .catch(() => {});
    void refresh();
      ...
      {(settings.serveWebUi || settings.remoteControlEnabled) && (
        <>
          ...
          <AddressRow
            ...
            url={remoteUiUrl("127.0.0.1", WEB_PORT, pairToken)}
            openable
          />
          {lanIp && (
            <AddressRow
              label={t("Phone remote (Wi-Fi)")}
              url={remoteUiUrl(lanIp, WEB_PORT, pairToken)}
            />
          )}
          <p className="text-[12px] leading-relaxed text-ink-muted">
            {t(
              "The phone remote link contains this computer's pairing key — open it on the phone as-is, and only share it with people you want controlling this Harbor.",
            )}
          </p>
          {webError && (
