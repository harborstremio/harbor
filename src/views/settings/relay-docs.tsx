    ...
    <div className="flex flex-col gap-8">
      ...
      <div ref={docsRef} className="flex flex-col gap-8">
        <header className="flex flex-col gap-2 border-b border-edge-soft pb-6">
          <p className="text-[10.5px] font-semibold uppercase tracking-[0.18em] text-accent">{t("Self-host")}</p>
        ...
        <DocsBlock>
          ...
          <DocsList>
            ...
            <li>
              {t("For the manual path:")} <DocsCode>node</DocsCode> {t("20+ and")} <DocsCode>wrangler</DocsCode>{" "}
              {t("CLI.")}
            </li>
          </DocsList>
        </DocsBlock>
        ...
        <DocsBlock>
          ...
          <DocsOl>
            ...
            <li>
              {t("Generate a Cloudflare API token with")} <DocsCode>Workers Scripts: Edit</DocsCode> {t("and")}{" "}
              <DocsCode>Account: Read</DocsCode> {t("permissions at")}{" "}
              <DocsCode>dash.cloudflare.com/profile/api-tokens</DocsCode>. {t("Paste it into Harbor.")}
            </li>
            <li>{t("Pick the Cloudflare account to deploy under.")}</li>
            <li>
              {t("Wait for the upload to finish. The relay URL gets written to")} <DocsCode>togetherRelayUrl</DocsCode>{" "}
              {t("in Harbor settings.")}
            </li>
          </DocsOl>
        </DocsBlock>
        ...
        <DocsBlock>
          ...
          <DocsP>{t("For users who want to deploy themselves or already have a wrangler workflow.")}</DocsP>
          <DocsOl>
            <li>
            ...
            <li>
              {t("In Harbor: Settings, Harbor Relay, then")} <DocsKbd>{t("Use a different URL")}</DocsKbd>.{" "}
              {t("Paste the URL with")} <DocsCode>wss://</DocsCode> {t("as the scheme instead of")}{" "}
        ...
        <DocsBlock>
          ...
          <DocsP>
            ...
            <DocsCode>workers.dev</DocsCode> {t("subdomain acts as the access token. There is no login.")}
          </DocsP>
          <DocsP>
...
function DocsH2({ children }: { children: React.ReactNode }) {
  return <h3 className="font-display text-[20px] font-medium tracking-tight text-ink">{children}</h3>;
}
