...
export function LanguageDropdown() {
  ...
  const [imgState, setImgState] = useState<"loading" | "recolor" | "original" | "missing">("loading");
  ...
  useEffect(() => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        setImgState(transparent ? "recolor" : "original");
      } catch {
        setImgState("original");
      }
    };
    img.onerror = () => {
      if (alive) setImgState("missing");
    };
    img.src = "/manga-language.png";
    return () => {
      alive = false;
    };
  }, []);
    ...
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen((v) => !v)} className={TRIGGER}>
        {imgState === "missing" ? (
          ...
          <span
            aria-hidden
            className="h-[22px] w-[22px] shrink-0 text-ink"
            style={{
              WebkitMaskImage: "url(/manga-language.png)",
              maskImage: "url(/manga-language.png)",
