  ...
  const cancelButton = (
    <button
      ...
      className="
        ...
        flex h-11 -translate-x-1/2 cursor-pointer
        items-center gap-2 rounded-full
        border border-white/15 bg-black/45 px-6
        text-[13.5px] font-medium text-white/75
    ...
    >
      ...
      {kid && (
        <div className="pointer-events-none absolute inset-0 overflow-hidden">
          {LOADER_BUBBLES.map((left, index) => (
            <span
              key={left}
              className="
                curfew-bubble absolute bottom-0
          ...
          <img
            ...
            draggable={false}
            className="
              absolute right-[18%] top-[18%]
              h-10 w-auto opacity-90
            "
          />
        </div>
      )}
