...
export function ListSettingsMenu({
  ...
  const contextRef = useContextTarget<HTMLButtonElement>(() => {
    ...
    return {
      kind: "actions",
      id: `list:${store.storageKey}:${list.id}`,
      label: list.name,
      isValid: () =>
        isMembershipProfileCurrent(profile) &&
        store.readLists().some((entry) => entry.id === list.id),
      actions: () => [
        {
      ...
      {renaming && (
        <RenameModal
          ...
          onSubmit={(name, description) => {
            ...
            const result = store.renameListWithResult(list.id, name, profile, description);
      ...
      {confirming && (
        <ConfirmDelete
          ...
          onConfirm={() => {
            ...
            const result = store.deleteListWithResult(list.id, profile);
            if (result.status === "error") {
              setError(t(membershipFailureMessage(result)));
              return;
            }
