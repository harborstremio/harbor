...
import { finishDiscordSignup, signInWithDiscord, startDiscordSignup } from "@/lib/account/discord-link";
...
const DISCORD_DEAD_CODES = new Set(["discord_code_invalid", "challenge_invalid", "discord_unreachable"]);
...
export function AccountAuthForm({ onRecovery }: { onRecovery?: (code: string) => void }) {
  ...
  const [discordPending, setDiscordPending] = useState<{ state: string; code: string } | null>(null);
  ...
  const submit = async () => {
    ...
    try {
      if (discordPending) {
        const { recoveryCode } = await finishDiscordSignup(discordPending.state, discordPending.code, trimmed, password);
      ...
      <div className="flex items-center gap-3.5 border-b border-edge-soft/70 px-6 pt-6 pb-5">
        ...
        <div className="flex min-w-0 flex-col">
          <h3 className="font-display text-[19px] font-medium tracking-tight text-ink">
            {discordPending ? t("Choose your username") : mode === "register" ? t("Join Harbor") : t("Welcome back")}
          </h3>
        ...
        >
          ...
          {(discordPending || mode === "register") && (
            <p className="flex items-start gap-2 text-[11.5px] leading-snug text-ink-subtle">
              ...
              {discordPending
                ? t("We'll show a one-time recovery key and send it to you on Discord. Save it: it's the only way back in if you forget your password.")
