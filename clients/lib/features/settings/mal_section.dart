...
/// The MyAnimeList section — PKCE authorization (open the authorize page, paste
/// the returned code) and the connected state. Reads [malConnectProvider].
/// Mirrors the web MyAnimeList settings.
class MalSection extends ConsumerStatefulWidget {
  const MalSection({super.key, required this.tokens});

  final HarborTokens tokens;

  @override
  ConsumerState<MalSection> createState() => _MalSectionState();
}

class _MalSectionState extends ConsumerState<MalSection> {
  final _codeController = TextEditingController();

  /// Web `confirmDisconnect`: the destructive MAL teardown is gated behind a
  ...
  Future<void> _codeOnPhone(MalConnectController ctrl) async {
    final code = await enterOnPhone(
      context,
      ref,
      label: 'MyAnimeList code',
  ...
  Future<void> _openAuthorize(MalConnectController ctrl) async {
  ...
  Widget build(BuildContext context) {
    ...
    final ctrl = ref.read(malConnectProvider.notifier);
    return SettingsSection(
      tokens: t,
      title: 'MyAnimeList',
      subtitle: tr.t('Connect MyAnimeList to sync your anime watch progress.'),
      children: [
        switch (state) {
          MalConnectDone(:final session) => _connected(t, ctrl, session),
          MalConnectIdle() => _connectButton(t, ctrl),
  ...
  Widget _connected(
    HarborTokens t,
    MalConnectController ctrl,
    MalSession session,
  ) {
    final tr = ref.watch(translationsProvider);
    final name = session.userName;
    final avatar = ref.watch(malAvatarProvider).asData?.value;
    ...
    return Column(
      ...
      children: [
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        Text(
                          tr.t('Disconnect from MyAnimeList'),
                          style: TextStyle(
                            color: t.ink,
                            fontSize: 13,
                            fontWeight: FontWeight.w600,
        ...
        SettingToggleRow(
          ...
          sub: tr.t(
            'Finishing an anime episode updates your MyAnimeList progress. '
            'Forward only: it never lowers a count you already have.',
          ),
          value: ref.watch(settingsProvider).getBool('malAutoSync'),
          onChanged: (v) => settingsCtrl.setValue('malAutoSync', v),
        ),
        if (avatar != null && avatar.isNotEmpty)
          SettingToggleRow(
            tokens: t,
            label: tr.t('Use MyAnimeList avatar'),
            sub: tr.t(
              'Set your MyAnimeList profile picture as your Harbor avatar.',
            ),
            value: ref.watch(settingsProvider).getBool('useMalAvatar'),
            onChanged: (v) => settingsCtrl.setValue('useMalAvatar', v),
          ),
      ],
    );
  }

  /// The destructive-action guard (web mal-panel `confirmDisconnect`): a warning
  /// with Cancel / Disconnect. Only the confirm button tears down.
    ...
    child: Row(
      children: [
        Expanded(
          child: Text(
            tr.t(
              'Disconnect MyAnimeList? Your progress will stop syncing until '
              'you reconnect.',
            ),
  ...
  Widget _connectButton(HarborTokens t, MalConnectController ctrl) => Focusable(
    tokens: t,
    scale: 1.0,
    borderRadius: 12,
    ...
    child: Container(
      ...
      child: Row(
        ...
        children: [
          ...
          const SizedBox(width: 10),
          Text(
            ref.watch(translationsProvider).t('Connect MyAnimeList'),
            style: TextStyle(
              color: t.accent,
              fontSize: 14,
              fontWeight: FontWeight.w600,
  ...
  Widget _pasteForm(
    HarborTokens t,
    MalConnectController ctrl,
    MalConnectState state,
  ) {
    final tr = ref.watch(translationsProvider);
    final submitting = state is MalConnectSubmitting;
    return _box(
      t,
      Column(
        ...
        children: [
          Text(
            tr.t(
              'Authorize Harbor in the MyAnimeList page that opened, then paste '
              'the code shown there below.',
            ),
                  ...
                  child: Center(
                    child: TvTextField(
                      ...
                      decoration: InputDecoration(
                        ...
                        hintText: tr.t('Paste the MyAnimeList code'),
