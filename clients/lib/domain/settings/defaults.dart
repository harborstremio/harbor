...
/// The default `Settings` object — a faithful transcription of Harbor's
/// `src/lib/settings/defaults.ts` `DEFAULT`. Every field and value matches the
/// web app so effective settings and Stremio-synced state behave identically.
/// Built as a function (not `const`) because `seekPreviewEnabled` depends on the
/// device core count, exactly as the web default keys off `hardwareConcurrency`.
Map<String, dynamic> buildDefaultSettings() => {
  ...
  // Rotate the detail hero through the title's backdrop gallery (web default
  // off). Only engages when there are >= 2 distinct backdrops and no pinned art.
  ...
  // Sound effects (web SoundEffects). soundTheme 'none' = off; sfxVolume is the
  // web 0-100 scale; playerVolumeSfx gates the in-player volume-change tone.
  'soundTheme': 'none',
  'sfxVolume': 50,
  ...
  // The "Stats" (Wrapped year-in-review) button in the Library header.
  'wrappedButton': true,
  ...
  'seekPreviewEnabled': (Platform.numberOfProcessors) >= 4,
  'instantPlay': true,
